...
const stat: any = function (filename) {
...
export default class Module {
    ...
    static _findPath(request: ModuleIdType, paths: string[], isMain: boolean) {
    ...
    static _resolveLookupPaths(request: string, parent, newReturn) {
        ...
        console.debug('RELATIVE: requested: %s set ID to: %s from %s', request, id,
            parent.id);
    ...
    static _resolveFilename(request: any, parent, isMain, options) {
        // if (NativeModule.nonInternalExists(request)) {
        //   return request;
        // }
        ...
        if (!filename) {
            ...
            var err: any = new Error(`Cannot find module '${request}'`);
...
function _nodeModulePaths_win32(from: string) {
...
function _nodeModulePaths_posix(from: string) {
    // guarantee that 'from' is absolute.
    from = path.resolve(from);
